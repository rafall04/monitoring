...
npm run typecheck                # tsc --noEmit across all 6 workspaces — the main gate
npm run build                    # NOTE: builds the frontend only (next build)
npm run dev                      # backend :4000 + worker :4100 + wabot :4200 + frontend :3000 (concurrently)
npm run dev:backend              # single process; also dev:worker / dev:wabot / dev:frontend
npm run seed                     # default accounts; SEED_DEMO=true also seeds demo data
```

- **Single-workspace typecheck:** `npm run typecheck -w @noc/backend` (or `@noc/worker`, `@noc/wabot`, `@noc/frontend`, `@noc/shared`, `@noc/server`).
...
### Four processes, one shared core
A monorepo of 6 workspaces. The split exists because Next.js can't host long-lived router TCP connections, background polling, a WebSocket server, or a WhatsApp session:
...
- **`apps/wabot`** — the WhatsApp bot (Baileys). Single instance ONLY (a WA session can't be sharded). Consumes the `noc:wa:outbox` Redis list (LPUSH by producers → BLPOP here, retry → `dead`), consumes `noc:wa:control` for admin ops (`logout` = unlink + wipe keys → fresh QR; `reconnect` = soft restart), publishes its session snapshot to `noc:wa:session` for the **Admin → WhatsApp** pairing UI (`whatsapp:manage` perm), and dispatches inbound commands (`LINK`, `KOMPLAIN`, `PROSES`/`SELESAI`, member `STATUS`/`LOGOUT`/`TIKET`/`INFO`, staff `SITES`/`DOWN`/`ACK`/`PING`/`TIKET`/`LAPORAN`). Complaint intake collects name → **department** → site → message (a linked member is asked for dept once, saved to `AppUser.department`); `POST/GET /me/tickets` is the member's web twin of `KOMPLAIN`/`TIKET`, sharing `createAndForwardTicket` (`@noc/server` `ticket-service`) so forwards look identical — `Ticket.reporterPhone` is nullable because web complaints may have no linked number. Command identity = verified `AppUser.phone`; staff commands reuse `siteScopeFor`/`canAccessSite`. Auth state lives encrypted in `wa_auth_key` — container rebuilds never re-pair. Recipients are `wa_recipient` rows — unified per-site list where each row is a `number` or a `group` (JID) with its own `alerts`/`tickets` flags. `WA_DRIVER=mock` runs the whole path without a WA number. Full doc: `docs/whatsapp-bot-plan.md` + README §WhatsApp bot.
- **`apps/frontend`** — Next.js App Router + Tailwind + TanStack Query + Leaflet.

### The status engine is the convergence point
`packages/server/src/status-engine.ts` is the **only** place a device status change is applied, so the **webhook path** (realtime, `apps/backend/src/routes/webhook.ts`) and the **poller path** (heartbeat/reconciliation, `apps/worker/src/poller.ts`) behave identically. On a *changed* status it transactionally updates the `Device` row + writes a `StatusEvent`, refreshes the Redis status cache, publishes `device.status` + a recomputed `site.summary`, and fires a Telegram alert for critical devices. An *unchanged* status only refreshes the Redis heartbeat. Recovery (`up`) clears incident ack metadata. The poller's Netwatch host matching (`applyDeviceStatusesByHost`) **excludes probe-owned devices** (`watchInterface`/`watchPort`/`watchNatDstPort` non-null) so a Netwatch entry on the same IP can never overwrite a probe verdict. When touching status logic, edit it here — not in the two callers.

### Probe-watch devices (interface / TCP port / NAT traffic)
A `Device` can carry a **probe watch** instead of plain Netwatch — three mutually-exclusive modes (setting one clears the others), all written by the worker's per-router poll (`poller.ts`) through the same status-engine:

- **Interface watch** (`watchInterface`) — status follows a RouterOS interface's `running` flag (`disabled`/not-running → `down`, absent → `unknown`, never a blind verdict); `StatusSource` `'interface'`. `GET /routers/:id/interfaces` feeds the live pick-list in `MarkerPanel`.
- **TCP-port watch** (`watchPort`) — the NOC server connects to `ipAddress:watchPort` (4 s timeout); refused/timeout → `down`, no IP → `unknown`. Catches "host up, service dead" (e.g. a killed dst-nat); `'tcp'`.
- **NAT-traffic watch** (`watchNatDstPort` + `watchNatStaleMin`, default 5 min) — the `dstnat` rule's `bytes` counter is truth: rule missing → `unknown`, **disabled → `down`** (the incident case), flat counter past the stale window → `down`, growing → `up`; the first read lands `unknown` honestly and a shrunken counter (reboot/recreate) re-baselines. Baseline lives in Redis `noc:device:<id>:natwatch`; `'traffic'`.

Alerts for all probe devices are gated by a **work-hours window**: `Setting.uplinkAlert{Start,End}Min/Days` global default, optional per-device `watchAlertWindow` JSON override (`AlertWindow` in `@noc/shared` — supports overnight windows). Status still updates 24/7; `notify.ts` suppresses the notification outside the window, and `uplinkWindowCatchUp` (`uplink.ts`) sends one "still down at window open" alert — deduped by a per-window-instance Redis flag. Containers run `TZ=Asia/Jakarta` (compose) — the window evaluates wall-clock time, so alpine images install `tzdata`. A device-less sibling, `RouterMikrotik.watchConfig` (firewall drift watch: per-poll nat/filter/mangle snapshot → diff → `router.config-change` audit row + `router.config` WS toast + Telegram/WA on server-relay sites), also runs inside the poll. Full ops doc: `docs/watch-modes.md`.

### Device status vs. manual override (two separate fields)
`Device.status` is the **raw** monitored truth (Netwatch or probe verdict) — `up | down | unknown` (`DeviceStatus`); the status-engine only ever writes these, tagging each change with a `StatusSource` (`webhook | polling | manual | interface | tcp | traffic`). `maintenance` is **not** a status — it's a separate `Device.manualOverride` field that **suppresses alarms and wins the displayed status** (a flapping host under maintenance never pages). Markers and alerting must fold the two together with `effectiveStatus(status, override)` (`@noc/shared` `types.ts`), which returns the `DisplayStatus` actually rendered (`up | down | unknown | maintenance | warning`) — never branch on `status` alone.

### Realtime flow (worker/webhook → browser)
status-engine **publishes** to Redis channel `noc:site:<id>:events` → the backend **WS hub** (`apps/backend/src/ws/hub.ts`) `psubscribe`s `noc:site:*:events` and fans each message out to sockets that joined that site's room → the frontend (`lib/ws.ts` `useSiteSocket`) patches the **TanStack Query cache per-device** (`lib/queries.ts` `applyWsEvent`) so one marker updates without a refetch. Channel/key names come from `@noc/shared` `events.ts` — never hardcode them. WS auth is the access JWT sent as a `Sec-WebSocket-Protocol` offer (`['bearer', <token>]` — a `?token=` query param would leak into request/proxy logs; the server selects `bearer` via `handleProtocols`); subscribe is re-checked against `canAccessSite`.
...
Inside one `pollRouter` pass the order is: router resource → Netwatch reconcile → interface watch → **config-drift watch** (`checkConfigDrift`, only when `router.watchConfig`) → **TCP probes** → **NAT-traffic probes**. The three auxiliary watches are individually `.catch`-wrapped — a probe bug logs a warning and must never fail the poll, or the circuit breaker would mark a healthy router offline.
...
**Same-origin proxy design (important):** in the browser the API base is the **empty string** — the Next.js server *rewrites* `/api/*` and `/uploads/*` to the backend over the internal network (`next.config.mjs`, `BACKEND_ORIGIN`). So the browser only ever talks to one origin → **no CORS, no separate public `api.` domain needed**. **Gotcha: rewrites are baked into `routes-manifest.json` at `next build`** — `BACKEND_ORIGIN` must be set as a Docker build arg (compose does this), not just a runtime env, and stray `apps/frontend/.env*` files must never reach the build context (`.dockerignore` covers `**/.env*`). **But Next rewrites can't proxy WebSocket upgrades**, so `lib/ws.ts` reaches the backend directly (`ws://<host>:<backendPort>/ws` for IP/localhost, or same-origin `wss://<domain>/ws` behind a tunnel that routes `/ws`). If `/ws` isn't routed, live updates degrade gracefully — the rest of the app still works over HTTP.
